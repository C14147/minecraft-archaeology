...
/* JADX INFO: loaded from: minecraft-rd-131655.jar:org/lwjgl/opengl/ContextAttribs.class */
public final class ContextAttribs {
    ...
    IntBuffer getAttribList() {
        ...
        if (this.majorVersion != 1 || this.minorVersion != 0) {
            map.put(Integer.valueOf(CONTEXT_MAJOR_VERSION_ARB), Integer.valueOf(this.majorVersion));
            map.put(Integer.valueOf(CONTEXT_MINOR_VERSION_ARB), Integer.valueOf(this.minorVersion));
        }
        if (this.contextFlags != 0) {
            map.put(Integer.valueOf(CONTEXT_FLAGS_ARB), Integer.valueOf(this.contextFlags));
        }
        if (this.profileMask != 0) {
            map.put(37158, Integer.valueOf(this.profileMask));
        }
        if (this.contextResetNotificationStrategy != 33377) {
            map.put(33366, Integer.valueOf(this.contextResetNotificationStrategy));
        }
        if (this.contextReleaseBehavior != 8344) {
            map.put(Integer.valueOf(CONTEXT_RELEASE_BEHABIOR_ARB), Integer.valueOf(this.contextReleaseBehavior));
        }
        if (this.layerPlane != 0) {
            map.put(Integer.valueOf(CONTEXT_LAYER_PLANE_ARB), Integer.valueOf(this.layerPlane));
        }
        if (map.isEmpty()) {
            return null;
        }
        ...
        for (Map.Entry<Integer, Integer> attrib : map.entrySet()) {
            attribs.put(attrib.getKey().intValue()).put(attrib.getValue().intValue());
        }
        attribs.put(0);
        attribs.rewind();
        return attribs;
