...
/* JADX INFO: loaded from: pre-Classic rd-20090515.jar:com/mojang/minecraft/phys/AABB.class */
public class AABB {
    ...
    public float clipXCollide(AABB c, float xa) {
        ...
        if (xa < 0.0f && c.x0 >= this.x1) {
            float max2 = (this.x1 - c.x0) + this.epsilon;
            if (max2 > xa) {
                xa = max2;
            }
        }
    ...
    public float clipYCollide(AABB c, float ya) {
        ...
        if (ya < 0.0f && c.y0 >= this.y1) {
            float max2 = (this.y1 - c.y0) + this.epsilon;
            if (max2 > ya) {
                ya = max2;
            }
        }
    ...
    public float clipZCollide(AABB c, float za) {
        ...
        if (za < 0.0f && c.z0 >= this.z1) {
            float max2 = (this.z1 - c.z0) + this.epsilon;
            if (max2 > za) {
                za = max2;
            }
        }
