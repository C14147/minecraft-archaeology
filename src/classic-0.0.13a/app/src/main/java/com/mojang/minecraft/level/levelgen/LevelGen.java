...
/* JADX INFO: loaded from: client.jar:com/mojang/minecraft/level/levelgen/LevelGen.class */
public class LevelGen {
    ...
    public boolean generateLevel(Level level, String userName, int width, int height, int depth) {
        ...
        double[] heightMap = buildHeightmap(width, height);
    ...
    public void carveTunnels() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            for (int l = 0; l < length; l++) {
                x = (float) (((double) x) + (Math.sin(dir1) * Math.cos(dir2)));
                    ...
                    for (int yy = (int) (y - size); yy <= ((int) (y + size)); yy++) {
                        for (int zz = (int) (z - size); zz <= ((int) (z + size)); zz++) {
                            ...
                            if (dd < size * size && xx >= 1 && yy >= 1 && zz >= 1 && xx < this.width - 1 && yy < this.depth - 1 && zz < this.height - 1) {
                                ...
                                if (this.blocks[ii] == Tile.rock.f47id) {
                                    this.blocks[ii] = 0;
                                }
                            }
                        }
    ...
    public void addWater() {
        ...
        long tiles = 0;
        int target = Tile.calmWater.f47id;
        for (int x = 0; x < this.width; x++) {
            tiles = tiles + floodFillLiquid(x, (this.depth / 2) - 1, 0, 0, target) + floodFillLiquid(x, (this.depth / 2) - 1, this.height - 1, 0, target);
        }
        for (int y = 0; y < this.height; y++) {
            tiles = tiles + floodFillLiquid(0, (this.depth / 2) - 1, y, 0, target) + floodFillLiquid(this.width - 1, (this.depth / 2) - 1, y, 0, target);
        }
        for (int i = 0; i < (this.width * this.height) / 5000; i++) {
            int x2 = this.random.nextInt(this.width);
            int y2 = (this.depth / 2) - 1;
            int z = this.random.nextInt(this.height);
            if (this.blocks[(((y2 * this.height) + z) * this.width) + x2] == 0) {
                tiles += floodFillLiquid(x2, y2, z, 0, target);
            }
        }
        long after = System.nanoTime();
        System.out.println("Flood filled " + tiles + " tiles in " + ((after - before) / 1000000.0d) + " ms");
    }
    ...
    public void addLava() {
        ...
        for (int i = 0; i < ((this.width * this.height) * this.depth) / 10000; i++) {
            ...
            if (this.blocks[(((y * this.height) + z) * this.width) + x] == 0) {
                lavaCount++;
                floodFillLiquid(x, y, z, 0, Tile.calmLava.f47id);
            }
        }
    ...
    public long floodFillLiquid(int x, int y, int z, int source, int tt) {
        ...
        int p = 0 + 1;
        this.coords[0] = (((y << hBits) + z) << wBits) + x;
        long tiles = 0;
            ...
            for (int xx = x0; xx < x1; xx++) {
                ...
                if (z0 < this.height - 1) {
                    ...
                    if (south && !lastSouth) {
                        ...
                        int i2 = p;
                        p++;
                        this.coords[i2] = cl + this.width;
                    }
                    lastSouth = south;
                }
                if (y0 > 0) {
                    byte b = this.blocks[cl - upStep];
                    if ((target == Tile.lava.f47id || target == Tile.calmLava.f47id) && (b == Tile.water.f47id || b == Tile.calmWater.f47id)) {
                        this.blocks[cl - upStep] = (byte) Tile.rock.f47id;
                    }
                    boolean below = b == source;
                    if (below && !lastBelow) {
                        if (p == this.coords.length) {
                            coordBuffer.add(this.coords);
                            this.coords = new int[1048576];
