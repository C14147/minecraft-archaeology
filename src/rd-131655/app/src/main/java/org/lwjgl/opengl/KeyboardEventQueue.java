...
/* JADX INFO: loaded from: minecraft-rd-131655.jar:org/lwjgl/opengl/KeyboardEventQueue.class */
final class KeyboardEventQueue extends EventQueue implements KeyListener {
    private static final int[] KEY_MAP = new int[LinuxKeycodes.XK_Delete];
    ...
    private void putKeyboardEvent(int key_code, byte state, int character, long nanos, boolean repeat) {
        this.event.clear();
        this.event.putInt(key_code).put(state).putInt(character).putLong(nanos).put(repeat ? (byte) 1 : (byte) 0);
        this.event.flip();
        putEvent(this.event);
    }
    ...
    private void putKeyEvent(int key_code, int key_location, byte state, int character, long nanos, boolean repeat) {
        ...
        int key_int_char = character & LinuxKeycodes.XK_Delete;
    ...
    public void keyReleased(KeyEvent e) {
        handleKey(e.getKeyCode(), e.getKeyLocation(), (byte) 0, 0, e.getWhen() * 1000000);
    }
