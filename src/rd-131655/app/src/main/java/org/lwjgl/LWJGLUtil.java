...
/* JADX INFO: loaded from: minecraft-rd-131655.jar:org/lwjgl/LWJGLUtil.class */
public class LWJGLUtil {
    ...
    /* JADX INFO: loaded from: minecraft-rd-131655.jar:org/lwjgl/LWJGLUtil$TokenFilter.class */
    ...
    static {
        ...
        if (osName.startsWith("Linux") || osName.startsWith("FreeBSD") || osName.startsWith("OpenBSD") || osName.startsWith("SunOS") || osName.startsWith("Unix")) {
            PLATFORM = 1;
        } else {
    ...
    private static String getPathFromClassLoader(final String libname, final ClassLoader classloader) {
        ...
        try {
            ...
            log("Failure locating " + e2 + " using classloader:" + c);
            return null;
        }
    }
    ...
    public static void log(CharSequence msg) {
        if (DEBUG) {
            System.err.println("[LWJGL] " + ((Object) msg));
        }
    }
    ...
    public static Map<Integer, String> getClassTokens(TokenFilter filter, Map<Integer, String> target, Iterable<Class> tokenClasses) {
        ...
        for (Class tokenClass : tokenClasses) {
            Field[] arr$ = tokenClass.getDeclaredFields();
            for (Field field : arr$) {
