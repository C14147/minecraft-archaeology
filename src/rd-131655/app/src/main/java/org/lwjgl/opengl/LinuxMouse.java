...
/* JADX INFO: loaded from: minecraft-rd-131655.jar:org/lwjgl/opengl/LinuxMouse.class */
final class LinuxMouse {
    ...
    private void setCursorPos(boolean grab, int x, int y, long nanos) {
        ...
        if (dx != 0 || dy != 0) {
            ...
            if (grab) {
                ...
                putMouseEventWithCoords((byte) -1, (byte) 0, x, y2, 0, nanos);
            }
        }
    }
    ...
    private void handleButtonPress(boolean grab, byte button, long nanos) {
        switch (button) {
            case 4:
                putMouseEvent(grab, (byte) -1, (byte) 0, 120, nanos);
                this.accum_dz += 120;
                break;
            case 5:
                putMouseEvent(grab, (byte) -1, (byte) 0, -120, nanos);
