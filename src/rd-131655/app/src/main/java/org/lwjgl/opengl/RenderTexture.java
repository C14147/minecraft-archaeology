...
/* JADX INFO: loaded from: minecraft-rd-131655.jar:org/lwjgl/opengl/RenderTexture.class */
public final class RenderTexture {
    ...
    public RenderTexture(boolean useRGB, boolean useRGBA, boolean useDepth, boolean isRectangle, int target, int mipmaps) {
        ...
        if (useRGB) {
            this.pixelFormatCaps.put(isRectangle ? WGL_BIND_TO_TEXTURE_RECTANGLE_RGB_NV : WGL_BIND_TO_TEXTURE_RGB_ARB);
            this.pixelFormatCaps.put(1);
            this.pBufferAttribs.put(WGL_TEXTURE_FORMAT_ARB);
            this.pBufferAttribs.put(WGL_TEXTURE_RGB_ARB);
        } else if (useRGBA) {
            this.pixelFormatCaps.put(isRectangle ? WGL_BIND_TO_TEXTURE_RECTANGLE_RGBA_NV : WGL_BIND_TO_TEXTURE_RGBA_ARB);
            this.pixelFormatCaps.put(1);
            this.pBufferAttribs.put(WGL_TEXTURE_FORMAT_ARB);
            this.pBufferAttribs.put(WGL_TEXTURE_RGBA_ARB);
        }
        if (useDepth) {
            this.pixelFormatCaps.put(isRectangle ? WGL_BIND_TO_TEXTURE_RECTANGLE_DEPTH_NV : WGL_BIND_TO_TEXTURE_DEPTH_NV);
            this.pixelFormatCaps.put(1);
            this.pBufferAttribs.put(WGL_DEPTH_TEXTURE_FORMAT_NV);
            this.pBufferAttribs.put(WGL_TEXTURE_DEPTH_COMPONENT_NV);
        }
        this.pBufferAttribs.put(WGL_TEXTURE_TARGET_ARB);
        this.pBufferAttribs.put(target);
        if (mipmaps != 0) {
            this.pBufferAttribs.put(WGL_MIPMAP_TEXTURE_ARB);
